...
class TaskModel():
    def __init__(self, id, today, due, work, week_day_work, week_end_work=0, days=0, gradient='+'):
        ...
        if (gradient == '-' and self.work/(self.due_date - today) < self.h) or (gradient == '+' and self.work/(self.due_date - today) > self.h):
            self.h = self.c
            self.c = (3*self.work/self.n - self.h)/2

        if gradient == '0':
            self.n = days if days != 0 else self.work / \
                self.work**(1/3) if self.work/self.work**(1/3) < self.due_date - \
    ...
    def c_for_huge(self, x):
        return (-8*x**3 - 4*self.h*x**2 + 16*self.work*x**2 + 2*self.h**2*x - 6*self.work **
                2*x + 4*self.h*self.work*x + self.h**3 - 3*self.h*self.work**2 - 2*self.h**2*self.work)/(3*self.work**2*(2*x+self.h))+1

    def model(self, x):
        return (self.h - self.c)/(3*self.work/(self.h+2*self.c))**2 * (x-self.due_date + 3*self.work/(self.h+2*self.c))**2 + self.c
